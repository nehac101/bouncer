...
# Lua script: will run automatically in Redis so it can't be interrupted mid execution
# keys will contain the redis keys to operate on, argv contains all other arguments
# Lua arrays are 1-indexed unlike Python arrays
...
class SlidingWindowLimiter:
    # bundles of redis commands are not atomic, hence needing lua script
    # redis is single-threaded by default but we need the lua script to have the 
